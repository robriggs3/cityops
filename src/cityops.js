var CityOps = (function () {
  ...
  function freePlaceId(cityData, name) {
    ...
    if (!Object.prototype.hasOwnProperty.call(taken, base)) return base;
    for (var n = 2; n < 500; n++) {
      var cand = base + '-' + n;
      if (!Object.prototype.hasOwnProperty.call(taken, cand)) return cand;
    }
  ...
  function planModel(data, state, nowIso) {
    ...
    var otherDays = stayIsos.filter(function (iso) { return iso !== today; }).map(function (iso) {
      return { iso: iso, label: dayLabel(iso),
        items: orderDayItems(byIso[iso] || [], dayItemOrderFor(state, iso)) };
    });
  ...
  function buildPlacePassPrompt(templateText, cityData, item, state) {
    ...
    var out = ['You are checking one place a traveler added by hand to an existing CityOps city guide.', ''];
    out = out.concat(cityHeaderLines(cityData));
    out.push('## The place', '');
  ...
  function revealJustMoved() {
    ...
    if (miss && miss.okText) {
      ...
      if (okn) okn.textContent = miss.okText;
    }
  ...
  // Sunset for the header, as {text, iso, isToday} or null. The date is
  // today when today falls inside the stay, and the arrival day otherwise, so
  // a guide opened three weeks early answers "what are the evenings like
  // there" rather than showing today's sunset for a city nobody is in.
  //
  // The TIME is rendered on the device's own clock. There is no timezone
  // database here and the city files carry no zone name, so the honest
  // options were this or nothing; this one is exactly right in the case the
  // feature exists for (a traveler standing in the city, whose phone has
  // already picked up the local zone) and the chip's tooltip says whose
  // clock it is, so it can never quietly mislead from a laptop back home.
  function sunsetChip(data, state) {
    ...
    return { text: pad2(d.getHours()) + ':' + pad2(d.getMinutes()), iso: iso, isToday: iso === today };
  }
  ...
  function renderSectionHeader(sv, state, totalItems) {
    ...
    // A task section never renders a section header (tabForSection sends it
    // to Plan), so this never offers to add a place to a to-do list.
    if (!isTaskSection(sv.section)) {
  ...
  function openAddPlaceModal(defaultSecId) {
    ...
    add.onclick = function () {
      ...
      var built = newPlaceDelta(data, {
        ...
        note: noteIn.value, day: daySel.value
      });
  ...
  return {
    ...
    sunKit: {
      ...
      cityLatLng: cityLatLng, julianDayUtc: julianDayUtc,
