var CityOps = (function () {
  ...
  function sunsetChip(data, state) {
    ...
    if (off !== null) {
      // Pure arithmetic, no Date: minutes past midnight in the city, wrapped
      // so a sunset that belongs to the neighbouring UTC day still reads as a
      // clock time rather than a negative number.
      //
      ...
      return { text: pad2(Math.floor(local / 60)) + ':' + pad2(local % 60),
        iso: iso, isToday: iso === today, cityClock: true };
    }
    ...
    return { text: pad2(d.getHours()) + ':' + pad2(d.getMinutes()),
      iso: iso, isToday: iso === today, cityClock: false };
  }
